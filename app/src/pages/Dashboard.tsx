      ...
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative flex flex-col items-center gap-4">
          <div className="relative">
            ...
            <div className="absolute inset-0 w-12 h-12 border-2 border-transparent border-t-cyan-500/50 rounded-full animate-spin" style={{ animationDuration: '2s', animationDirection: 'reverse' }} />
          </div>
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white font-sans selection:bg-indigo-500/30 selection:text-indigo-200 relative overflow-x-hidden">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/40 uppercase tracking-widest font-medium hidden sm:block">Pro</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Dialog open={showTemplates} onOpenChange={setShowTemplates}>
                <DialogTrigger asChild>
                  <Button className="relative group bg-white text-black hover:bg-white/90 rounded-full px-5 font-medium shadow-lg shadow-white/10 hover:shadow-white/20 transition-all overflow-hidden">
                ...
                <DialogContent className="max-w-2xl bg-[#0f0f14] border-white/10 text-white shadow-2xl shadow-black/50 rounded-2xl overflow-hidden">
                  ...
                  <DialogHeader className="relative">
                    <DialogTitle className="text-2xl font-bold text-white">Create new form</DialogTitle>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 relative">
                    <motion.button
                      ...
                      className="group p-6 border border-dashed border-white/10 rounded-2xl hover:border-indigo-500/50 hover:bg-white/5 transition-all text-center relative overflow-hidden"
                    >
                      <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                      <div className="relative">
                        <div className="w-14 h-14 mx-auto mb-4 bg-white/5 rounded-2xl flex items-center justify-center group-hover:bg-indigo-500/20 group-hover:scale-110 transition-all duration-300 border border-white/5 group-hover:border-indigo-500/30">
                          <Plus className="w-7 h-7 text-white/40 group-hover:text-indigo-400" />
                        </div>
                        <p className="font-semibold text-white mb-1">Blank Form</p>
                        <p className="text-sm text-white/40">Start from scratch</p>
                      </div>
                    </motion.button>

                    {[
                      { name: "Event Registration", icon: Users, color: "from-blue-500/20 to-indigo-500/20", border: "border-blue-500/30", iconColor: "text-blue-400" },
                      { name: "Customer Feedback", icon: Activity, color: "from-amber-500/20 to-orange-500/20", border: "border-amber-500/30", iconColor: "text-amber-400" },
                      { name: "Job Application", icon: FolderOpen, color: "from-emerald-500/20 to-teal-500/20", border: "border-emerald-500/30", iconColor: "text-emerald-400" },
                    ].map((template) => (
                      <motion.button
                        key={template.name}
                        whileHover={{ scale: 1.02 }}
                        ...
                        className="group p-6 border border-white/10 rounded-2xl hover:border-white/20 hover:bg-white/5 transition-all text-left relative overflow-hidden bg-white/[0.02]"
                      >
                        <div className={`absolute inset-0 bg-gradient-to-br ${template.color} opacity-0 group-hover:opacity-100 transition-opacity`} />
                        <div className="relative">
                          <div className={`w-14 h-14 bg-gradient-to-br ${template.color} rounded-2xl flex items-center justify-center mb-4 border ${template.border} group-hover:scale-110 transition-transform duration-300`}>
                            <template.icon className={`w-7 h-7 ${template.iconColor}`} />
                          </div>
                          <p className="font-semibold text-white mb-1">{template.name}</p>
                          <p className="text-sm text-white/40">Pre-built template</p>
      ...
      <main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-white/40 text-lg">Manage your forms and analyze responses</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
          {[
            ...
            { 
              ...
              value: forms.reduce((acc, form) => acc + form.responseCount, 0).toLocaleString(), 
              icon: BarChart3, 
              trend: "+8.2%",
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-white/40">{filteredForms.length} forms</span>
          </div>
        ...
        {filteredForms.length === 0 ? (
          ...
          >
            ...
            <div className="relative">
              ...
              <h3 className="text-xl font-semibold text-white mb-3">No forms yet</h3>
              ...
              {filteredForms.map((form, index) => (
                <motion.div
                  ...
                  className={
                    viewMode === "grid"
                      ...
                      : "group flex items-center gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:border-white/10 hover:bg-white/[0.04] transition-all cursor-pointer"
                  }
                ...
                >
                  {viewMode === "grid" ? (
                    ...
                    <>
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500/20 to-cyan-500/20 border border-indigo-500/20 flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-1 text-sm text-white/40 w-20 justify-end">
      ...
      <AlertDialog open={!!formToDelete} onOpenChange={() => setFormToDelete(null)}>
        <AlertDialogContent className="bg-[#0f0f14] border-white/10 text-white shadow-2xl rounded-2xl max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50 leading-relaxed">
              Are you sure you want to delete this form? This action cannot be undone and all responses will be permanently lost.
            </AlertDialogDescription>
          </AlertDialogHeader>
